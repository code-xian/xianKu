    ...
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="init()">
            ...
            <el-form-item label="出库单号" >
                <el-input  placeholder="请输入出库单号" clearable v-model="dataForm.rukuId"></el-input>
            </el-form-item>
            <el-form-item>
        ...
        <div class="tableData">
            <el-table
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <div slot="reference">
                            {{ scope.row.rukuStatus=="12"?"未审核":scope.row.rukuStatus=="10"?"审核通过":scope.row.rukuStatus=="11"?"审核未通过":"" }}
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                        ...
                        label="操作">
                    <template slot-scope="scope">
                        <el-button type="primary" size="mini"  @click="audit(scope.row.rukuId,true,scope.row.rukuMoney)" v-if="scope.row.rukuStatus==12">审核</el-button>
                        <el-button type="primary" size="mini"  @click="audit(scope.row.rukuId,false,scope.row.rukuMoney)" v-if="scope.row.rukuStatus==10||scope.row.rukuStatus==11">详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
        ...
        <in-audit  v-if="auditVisible" ref="audit" @refreshDataList="init"></in-audit>
    </div>
</template>

<script>
    import InAudit from "./InAudit";
    export default {
        name: "InList",
        data() {
            return {
