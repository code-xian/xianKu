    ...
    <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="init()">
            ...
            <el-form-item label="收货单号" >
                <el-input  placeholder="请输入收货单号" clearable v-model="dataForm.shouhuoId"></el-input>
            </el-form-item>
            <el-form-item>
        ...
        <div class="tableData">
            <el-table
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <div slot="reference">
                            {{ scope.row.shouhuoStatus=="12"?"未审核":scope.row.shouhuoStatus=="10"?"审核通过":scope.row.shouhuoStatus=="11"?"审核未通过":"" }}
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                        ...
                        label="操作">
                    <template slot-scope="scope">
                        <el-button type="primary" size="mini"  @click="audit(scope.row.shouhuoId,true,scope.row.purchaseAmount)" v-if="scope.row.shouhuoStatus==12">审核</el-button>
                        <el-button type="primary" size="mini"  @click="audit(scope.row.shouhuoId,false,scope.row.purchaseAmount)" v-if="scope.row.shouhuoStatus==10||scope.row.shouhuoStatus==11">详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
        ...
        <shouhuo-audit  v-if="auditVisible" ref="audit" @refreshDataList="init"></shouhuo-audit>
    </div>
</template>

<script>
    import ShouhuoAudit from "./ShouhuoAudit";
    export default {
        name: "ReceiveList",
        data() {
            return{
            ...
            getDataList() {
                ...
                this.$http({
                    ...
                    params: this.$http.adornParams({
                        shouhuoId:this.dataForm.shouhuoId,
